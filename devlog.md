...
| 29 | 2026-04-16 | ILI9341 顯示上下顛倒：MADCTL 0xE8 → 0x28，toggle MY+MX 實現 180° 翻轉 |
| 30 | 2026-04-16 | ILI9341 加上左右鏡像：MADCTL 0x28 → 0xA8，toggle MY 實現水平翻轉 |
| 31 | 2026-04-16 | GUI 文字反鏡像：counter-mirror glyph 與字元順序，抵消 LCD MADCTL 水平鏡像，文字恢復正向可讀 |
| 32 | 2026-04-16 | 改由 OV7670 處理畫面翻轉：還原 ILI9341 MADCTL=0xE8 與 gui.c，MVFP 0x37 → 0x27 |
...
---
## 29. ILI9341 顯示上下顛倒 — 2026-04-16

**原因 (Why)**
使用者要求將 LCD 畫面上下顛倒（180° 旋轉），配合實際機構安裝方向。

**處理方式 (How)**
- 修改 [App/Src/ili9341.c:176](App/Src/ili9341.c#L176) MADCTL 值
- 原本 `0xE8` (MY=1, MX=1, MV=1, BGR=1) → 改為 `0x28` (MV=1, BGR=1)
- 同時 toggle MY 與 MX 兩個 bit，等效於 180° 旋轉；保留 MV 維持 landscape、保留 BGR 色彩順序
- 座標系統 (240x320 → 320x240 landscape) 維持不變，畫面尺寸不受影響

**驗證 (Verification)**
- 燒錄後觀察 LCD 內容是否上下左右同時翻轉
- OV7670 預覽、GUI overlay、追蹤框應全部同步翻轉

---
## 30. ILI9341 加上左右鏡像 — 2026-04-16

**原因 (Why)**
在 180° 翻轉 (#29) 之後，使用者要求再加上左右鏡像效果。

**處理方式 (How)**
- 修改 [App/Src/ili9341.c:176](App/Src/ili9341.c#L176) MADCTL
- `0x28` → `0xA8`：設定 MY=1（在 MV=1 landscape 模式下等效水平鏡像）
- 最終 bits：MY=1, MX=0, MV=1, BGR=1

---
## 31. GUI 文字反鏡像 — 2026-04-16

**原因 (Why)**
#30 啟用 LCD 水平鏡像 (MADCTL=0xA8) 後，相機影像方向正確但 GUI 文字（WB / FPS 等）一併被鏡像，變成無法閱讀的倒字。

**處理方式 (How)**
在 [App/Src/gui.c](App/Src/gui.c) 對字元渲染做「反鏡像」，抵消 LCD 硬體鏡像：
- `GUI_DrawChar`：繪製 glyph 時使用 `mcol = FONT_W-1-col` 反轉欄位，gap column 改畫在左側
- `GUI_DrawString`：先算字串長度 N，字元 i 放在 `x + (N-1-i)*CHAR_W`，使字元在鏡像螢幕上由左至右閱讀

相機影像與 LCD 設定保持不變（MADCTL=0xA8、MVFP=0x37）。

---
## 32. 改由 OV7670 處理畫面翻轉 — 2026-04-16

**原因 (Why)**
#29~#31 透過 LCD MADCTL 與 GUI 反鏡像實作畫面翻轉，會導致文字跟著鏡像或需反向渲染。改由 sensor 端做翻轉，LCD 與 GUI 保持原樣，文字自然正向。

**處理方式 (How)**
- [App/Src/ili9341.c:176](App/Src/ili9341.c#L176) MADCTL 還原 `0xA8` → `0xE8`
- [App/Src/gui.c](App/Src/gui.c) `GUI_DrawChar` / `GUI_DrawString` 還原為原本正向渲染
- [App/Src/ov7670.c:162](App/Src/ov7670.c#L162) MVFP `0x37` → `0x27`（bit5 mirror 保持 1、bit4 vflip 由 1→0），相對原本 0x37 等效於「180° + 水平鏡像」
